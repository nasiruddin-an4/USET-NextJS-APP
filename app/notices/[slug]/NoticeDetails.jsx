"use client"
import { Calendar, Tag, ArrowLeft, FileText, Clock, Share2 } from "lucide-react"
    ...
    <div className="bg-slate-50">
      ...
      <div className="container mx-auto px-6 py-20 space-y-12">
        ...
        <div className="space-y-6">
          {/* Title */}
          <div className="">
            <h1 className="text-2xl md:text-4xl font-bold text-brandblue leading-tight text-balance mb-2">{notice.title}</h1>
            {/* Meta Information */}
          <div className="flex flex-wrap gap-2 items-center">
              <Calendar className="w-5 h-5 text-slate-600" />
                <p className="text-xs text-slate-500 uppercase tracking-wide">Published:</p>
                <p className="text-sm font-semibold text-slate-900">{notice.date}</p>
          </div>
          </div>
        </div>
        ...
        <div className="prose prose-lg max-w-none">
          <div className="space-y-6 text-slate-700 leading-relaxed">
              <p className="whitespace-pre-line text-base md:text-lg">{notice.content}</p>
            </div>
        </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200/50 rounded-2xl p-8 md:p-10">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-blue-600 font-semibold uppercase tracking-wide">Full Document</p>
                  <p className="text-slate-900 font-semibold">Download PDF for complete details</p>
                </div>
              </div>
              <a
                href={notice.pdf}
